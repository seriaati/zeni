...
// Keep legacy alias for TagPicker
type TagDropdownPos = DropdownPos;
...
function CategoryPicker({ selectedId, allCategories, onSelect, onCreateAndSelect }: CategoryPickerProps) {
  ...
  const filtered = query.trim()
    ? allCategories.filter((c) => c.name.toLowerCase().includes(query.toLowerCase()))
    : allCategories;

  const trimmed = query.trim();
  const exactMatch = allCategories.some((c) => c.name.toLowerCase() === trimmed.toLowerCase());
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (focusedIndex < filtered.length) {
          ...
          if (cat) { onSelect(cat.id); setOpen(false); }
        } else if (showCreate) {
          handleCreate();
        }
        break;
      ...
      <ul ref={listRef} style={{ listStyle: 'none', padding: 4, maxHeight: 200, overflowY: 'auto', margin: 0 }}>
        ...
        {filtered.map((cat, i) => (
          ...
          >
            ...
            />
            <span style={{ flex: 1, color: cat.id === selectedId ? 'var(--forest)' : 'var(--ink)' }}>{cat.name}</span>
            {cat.id === selectedId && <Check size={13} style={{ color: 'var(--forest)', flexShrink: 0 }} />}
          </li>
        ))}
        {showCreate && (
          <li
        ...
        {selected ? (
          <>
            <CategoryIcon
              ...
              containerSize={22}
              borderRadius={5}
              fallbackLetter={selected.name[0]}
            />
